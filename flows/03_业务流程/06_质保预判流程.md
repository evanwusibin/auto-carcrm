...
flowchart TD
    ...
    J --> J1{① 时间判断<br/>是否超过质保年限}
    J --> J2{② 里程判断<br/>是否超过质保里程}
    J --> J3{③ 保养判断<br/>是否按时完成保养}
    J --> J4{④ 免责判断<br/>故障描述是否含免责词}
    ...
    N -- 时间或里程超保 --> O1[likely_out_of_warranty<br/>初步判断可能超保]
    N -- 含免责条件 --> O2[manual_review_required<br/>需人工复核]
    N -- 保养不合规 --> O2
    N -- 条件均满足 --> O3[likely_in_warranty<br/>初步判断可能在保]
    N -- 信息不足 --> O4[insufficient_info<br/>信息不足无法判断]
    ...
    R --> S[⚠️ 本结果仅为初步预判<br/>最终质保结论以授权服务站检测为准]

    S --> T{用户下一步}
    T -- 了解了 --> U[结束]
    T -- 需要报修 --> V[进入报修流程<br/>预判结果带入报修单]
    T -- 有疑问 --> W[继续问答]
```
