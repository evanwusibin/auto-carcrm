...
| 向量检索 | Milvus 混合搜索（稠密+稀疏） | 与主库统一，支持向量+元数据联合过滤 |
| RAG 框架 | LangGraph | 快速编排文档加载/切分/检索/生成链路 |
| Embedding | bge-large-zh | 中文语义强，可本地部署，数据不出企业 |
| LLM | mimo-v2.5-pro | 中文理解好，API 成本低，指令遵循能力强 |
| 文档解析 | MinerU | 覆盖 PDF/Word/Excel 全格式 |
...
| 检索路径 | 技术实现 | 解决的问题 |
|---|---|---|
| 向量语义检索 | Milvus 混合搜索（稠密+稀疏） | 口语化描述、模糊表达 |
...
> 
> 针对商用车售后知识分散、查询低效、服务口径不统一的痛点，设计并参与搭建基于 RAG 的智能售后知识助手。完成文档解析、Chunk 切分、Embedding 向量化、四路混合检索（向量语义 + 关键词精确 + 结构化查询 + 案例检索）、Prompt 工程和答案引用溯源的完整链路。技术栈：FastAPI + MongoDB + Milvus 混合搜索 + bge-large-zh + mimo-v2.5-pro。
> 
