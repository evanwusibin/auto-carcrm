...
def call_llm_deal_hyde(rewritten_query):
    ...
    hyde_answer = chains.invoke(message)
    return hyde_answer


# 执行带过滤的混合向量检索
def search_chunks_by_hyde(hyde_answer,rewritten_query,item_names):
    ...
    ann_reqs = milvus_gateway.create_requests(
        ...
        expr= f"item_name in {item_names}",limit=5*2
    )
    # 固定格式
    milvus_result = milvus_gateway.hybrid_search(
        ...
        norm_score=True,
        limit = 5,
        output_fields=["chunk_id","item_name","parent_title","part","title","file_title","content"],
    )
    # 结果需要遍历return才行对吧
...
def normalize_retrieved_chunk(milvus_response:list[dict]):
    final_list_dict = []
...
def search_by_embedding(state: QueryGraphState) -> QueryGraphState:
    ...
    milvus_response = search_chunks_by_hyde(hyde_answer = hyde_answer,rewritten_query=rewritten_query,item_names=item_names)
